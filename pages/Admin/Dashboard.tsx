...
// ✅ NUEVO: parse de dinero tolerante ($, comas, strings, undefined)
const parseMoney = (v: any) => {
  if (v === null || v === undefined) return 0;
  const s = String(v).replace(/[^\d.]/g, ''); // quita $ , MXN etc
  const n = Number(s);
  return Number.isFinite(n) ? n : 0;
};

// ✅ NUEVO: extrae presupuesto/currency aunque el backend mande otro nombre
const getLeadBudget = (lead: Lead) => {
  const anyLead = lead as any;

  const amount = parseMoney(
    anyLead?.budget ??
    anyLead?.budgetAmount ??
    anyLead?.presupuesto ??
    anyLead?.budget_approx ??
    anyLead?.budgetApprox ??
    anyLead?.approxBudget ??
    anyLead?.presupuestoAprox
  );

  const currency =
    (anyLead?.currency ??
      anyLead?.budgetCurrency ??
      anyLead?.budget_currency ??
      anyLead?.moneda ??
      'MXN') as string;
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-8">
        ...
        {activeTab === 'leads' && (
          <div className="space-y-4">
            ...
            {leads.map(l => {
              const { amount, currency } = getLeadBudget(l);
              return (
                <div
                  key={l.id}
            ...
            {leads.length === 0 && (
              <div className="text-center text-gray-500 py-20 border-2 border-dashed rounded-3xl">Aún no hay leads.</div>
            )}
          </div>
        )}
