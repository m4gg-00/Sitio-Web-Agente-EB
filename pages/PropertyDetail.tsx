...
const PropertyDetail: React.FC = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setIsLoading(true);
      try {
        const [allProps, prof] = await Promise.all([apiService.getProperties(false), apiService.getProfile()]);
        const found = allProps.find(p => p.id === id);
        if (found) {
          setProperty(found);
          // Asegura índice válido si cambia el arreglo de imágenes
          setActiveImg(prev => {
            const len = (found.images || []).length;
            if (!len) return 0;
  ...
  const safeActive = hasImages ? Math.min(activeImg, images.length - 1) : 0;
    ...
    <div className="pt-20 pb-32 bg-white dark:bg-gray-900 text-gray-900 dark:text-white transition-colors min-h-screen">
      ...
      <div className="bg-gray-50 dark:bg-gray-800 py-4 border-b border-gray-100 dark:border-gray-700">
        <div className="max-w-7xl mx-auto px-4 flex justify-between items-center">
          <Link
            to="/propiedades"
            className="flex items-center text-sm font-semibold text-gray-600 dark:text-gray-300 hover:text-[#800020] dark:hover:text-[#ff3b5c]"
          >
          ...
          <div className="lg:col-span-8">
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                {/* ✅ Preview más grande + flechas */}
                <div className="relative bg-gray-100 dark:bg-gray-800 rounded-3xl overflow-hidden shadow-lg border border-gray-100 dark:border-gray-700">
                  {/* Cambié el alto para que se vea más grande y no se “recorte” tanto */}
                  <div className="h-[62vh] min-h-[360px] max-h-[640px]">
                    <img
                      src={hasImages ? images[safeActive] : 'https://via.placeholder.com/1200x800?text=No+Image'}
                      alt={property.title}
                  ...
                  {hasImages && images.length > 1 && (
                    <>
                      <button
                        type="button"
                        onClick={goPrev}
                        className="absolute left-4 top-1/2 -translate-y-1/2 p-2 rounded-full bg-white/80 hover:bg-white text-gray-900 shadow-lg border border-gray-200 backdrop-blur dark:bg-black/40 dark:hover:bg-black/60 dark:text-white dark:border-white/10"
                      ...
                      <button
                        type="button"
                        onClick={goNext}
                        className="absolute right-4 top-1/2 -translate-y-1/2 p-2 rounded-full bg-white/80 hover:bg-white text-gray-900 shadow-lg border border-gray-200 backdrop-blur dark:bg-black/40 dark:hover:bg-black/60 dark:text-white dark:border-white/10"
                      ...
                      {/* Contador */}
                      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full text-xs font-bold bg-black/60 text-white">
                ...
                <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
                  {images.map((img, idx) => (
                    <button
                      ...
                      className={`relative flex-shrink-0 w-24 h-24 rounded-xl overflow-hidden border-2 transition-all ${safeActive === idx
                        ? 'border-[#800020] ring-4 ring-[#800020]/10 dark:ring-[#ff3b5c]/10'
                        : 'border-transparent'
                        }`}
              ...
              <div className="py-6 border-b border-gray-100 dark:border-gray-800">
                <div className="flex items-center gap-3 mb-4">
                  <span
                    className={`px-3 py-1 text-xs font-bold uppercase rounded-full ${property.status === 'Disponible'
              ...
              <div className="py-6">
                ...
                <p className="text-gray-600 dark:text-gray-300 leading-relaxed whitespace-pre-line text-lg">{property.description}</p>
              </div>
            ...
            <div className="sticky top-32 space-y-6">
              <div className="bg-[#111827] dark:bg-black text-white p-8 rounded-3xl shadow-2xl border border-transparent dark:border-gray-800">
                ...
                <div className="text-4xl font-bold mb-4">
                  {formattedPrice} <span className="text-xl font-normal text-gray-400">{property.currency}</span>
                </div>
                ...
                <div className="mt-8 pt-8 border-t border-white/10 flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-gray-400 mt-1">Contacto: {profile?.whatsapp || CONTACT_INFO.whatsapp}</p>
                  </div>
                </div>
              </div>
            </div>
